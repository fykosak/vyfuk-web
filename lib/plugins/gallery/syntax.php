...
class syntax_plugin_gallery extends DokuWiki_Syntax_Plugin {
    ...
    function _gallery($data) {
        ...
        // build gallery
        if ($data['_single']) {
            $ret .= '<div class="offset-xl-4 col-xl-4 offset-lg-3 col-lg-6 offset-md-3 col-md-6 mb-3">';
            ...
            foreach ($files as $img) {
                ...
                if ($data['paginate'] && ($i % $data['paginate'] == 0)) {
                    $ret .= '<div class="row gallery_page gallery__' . $data['galid'] . '" id="gallery__' .
                        $data['galid'] . '_' . (++$page) . '">';
                    $close_pg = true;
                }
        ...
        if ($page) {
            ...
            for ($j = 1; $j <= $page; $j++) {
                $pgret .= '<a href="#gallery__' . $data['galid'] . '_' . $j . '" class="gallery_pgsel button">' . $j .
                    '</a> ';
            }
            $pgret .= '</div>';
        }

        return '<div class="row gallery' . $align . '">' . $pgret . $ret . '<div class="clearer"></div></div>';
    }
