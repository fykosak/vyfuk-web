...
class action_plugin_discussion extends DokuWiki_Action_Plugin {
    ...
    public function register(Doku_Event_Handler $contr) {
        $contr->register_hook(
                'ACTION_ACT_PREPROCESS', 'BEFORE', $this, 'handle_act_preprocess', array()
        );
        $contr->register_hook(
                'TPL_ACT_RENDER', 'AFTER', $this, 'comments', array()
        );
        $contr->register_hook(
                'INDEXER_PAGE_ADD', 'AFTER', $this, 'idx_add_discussion', array()
        );
        $contr->register_hook(
                'INDEXER_VERSION_GET', 'BEFORE', $this, 'idx_version', array()
        );
        $contr->register_hook(
                'PARSER_METADATA_RENDER', 'AFTER', $this, 'update_comment_status', array()
        );
        $contr->register_hook(
                'TPL_METAHEADER_OUTPUT', 'BEFORE', $this, 'handle_tpl_metaheader_output', array()
        );
        $contr->register_hook(
                'TOOLBAR_DEFINE', 'AFTER', $this, 'handle_toolbar_define', array()
        );
        $contr->register_hook(
                'AJAX_CALL_UNKNOWN', 'BEFORE', $this, 'handle_ajax_call', array()
        );
        $contr->register_hook(
                'TPL_TOC_RENDER', 'BEFORE', $this, 'handle_toc_render', array()
        );
    }
    ...
    public function handle_toolbar_define(Doku_Event $event, $param) {
        ...
        if ($this->_hasDiscussion($title) && $this->getConf('wikisyntaxok')) {
            ...
            foreach ($event->data as $btn) {
                if ($btn['type'] == 'mediapopup')
                    continue;
                if ($btn['type'] == 'signature')
                    continue;
                if ($btn['type'] == 'linkwiz')
                    continue;
                if ($btn['type'] == 'NewTable')
                    continue; //skip button for Edittable Plugin
                if (preg_match("/=+?/", $btn['open']))
                    continue;
    ...
    protected function _show($reply = null, $edit = null) {
        ...
        if (!$INFO['exists'])
            return false;
        if (!@file_exists($file) && !$this->isDiscussionEnabled())
            return false;
        if (!$_SERVER['REMOTE_USER'] && !$this->getConf('showguests'))
            return false;
        ...
        if (@file_exists($file)) {
            ...
            if (!$data['status'])
                return false; // comments are turned off
        ...
        if (isset($data['comments'])) {
            ...
            foreach ($data['comments'] as $key => $value) {
                if ($key == $edit)
                    $this->_form($value['raw'], 'save', $edit); // edit form
                else
    ...
    protected function _add($comment, $parent) {
        ...
        if ((!$this->getConf('allowguests')) && ($comment['user']['id'] != $_SERVER['REMOTE_USER'])
        ) {
        ...
        if (!@file_exists($file)) {
            ...
            $data = unserialize(io_readFile($file, false));
            if ($data['status'] != 1)
                return false; // comments off or closed
        }
    ...
    public function save($cids, $raw, $act = NULL) {
        ...
        if (!$cids)
            return false; // do nothing if we get no comment id
        ...
        foreach ($cids as $cid) {
            ...
            if (($user != $_SERVER['REMOTE_USER']) && (!$this->helper->isDiscussionMod()))
                return false;
    ...
    protected function _print($cid, &$data, $parent = '', $reply = '', $visible = true) {
        ...
        if ($comment['parent'] != $parent)
            return true;    // reply to an other comment
    ...
    protected function _use_avatar() {
        if (is_null($this->use_avatar)) {
            $this->use_avatar = $this->getConf('useavatar') && (!plugin_isdisabled('avatar')) && ($this->avatar = & plugin_load('helper', 'avatar'));
        }
    ...
    protected function _notify($comment, &$subscribers) {
        ...
        if (actionOK('subscribe')) {
            ...
            if (class_exists('Subscription')) { /* Introduced in DokuWiki 2013-05-10 */
                trigger_event(
                        'COMMON_NOTIFY_ADDRESSLIST', $data, array(new Subscription(), 'notifyaddresses')
                );
            } else { /* Old, deprecated default handler */
                trigger_event(
                        'COMMON_NOTIFY_ADDRESSLIST', $data, 'subscription_addresslist'
                );
            }
            $to = $data['addresslist'];
            if (!empty($to)) {
    ...
    protected function _countReplies(&$data, $rids) {
        ...
        foreach ($rids as $rid) {
            if (!isset($data['comments'][$rid]))
                continue; // reply was removed
            if (!$data['comments'][$rid]['show'])
                continue;
            $number++;
...
// do nothing if logged in user and no reCAPTCHA required
    ...
    public function idx_add_discussion(Doku_Event $event, $param) {
        ...
        if ((!$data['status']) || ($data['number'] == 0))
            return; // comments are turned off
...
// now add the comments
    ...
    protected function _addCommentWords($cid, &$data, $parent = '') {
        ...
        if ($comment['parent'] != $parent)
            return '';   // reply to an other comment
        if (!$comment['show'])
